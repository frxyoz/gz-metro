...
def in_gz_bbox(geom) -> bool:
    ...
    c = geom.centroid
    return GZ_LON[0] <= c.x <= GZ_LON[1] and GZ_LAT[0] <= c.y <= GZ_LAT[1]
...
def main() -> None:
    ...
    for row in phases:
        ...
        if start_name is None:
            ...
            out_line_feats.append({
                ...
                "properties": {"kind": "line", "line": csv_line,
        ...
        if best_result is None:
            # Multi-branch fallback: if each endpoint is on a DIFFERENT branch within
            # MAX_SNAP_M, emit all GZ-area branches for this line as a MultiLineString.
            best_start_b, best_start_d = None, float("inf")
            best_end_b,   best_end_d   = None, float("inf")
            for pt_a in start_pts:
                pt_a_utm = _to_utm(pt_a)
                for b in all_branches_utm:
    ...
    for feat in out_line_feats:
        ...
        chunk_data.append((g_utm, feat["properties"]["opened"], feat["properties"]["color"]))

    out_stn_feats: list[dict] = []
    seen_stn_keys: set = set()
    ...
    for feat in osm_stn_feats:
        ...
        for chunk_geom, chunk_opened, chunk_color in chunk_data:
        ...
        # Deduplicate: same (name_zh or name_en) + coordinates at ±0.001° (~100m)
        dedup_key = (name_zh or name_en, round(c[0], 3), round(c[1], 3))
        if dedup_key in seen_stn_keys:
            continue
        seen_stn_keys.add(dedup_key)

        out_stn_feats.append({
            "type": "Feature",
